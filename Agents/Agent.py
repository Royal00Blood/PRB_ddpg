...
class PRB_DDPG_Agent:
    ...
    def update(self):
        ...
        critic1_loss = nn.MSELoss(reduction='none')(self.critic1(action=actions,state=states), target_q_values.detach().unsqueeze(1)).mean()
        critic2_loss = nn.MSELoss(reduction='none')(self.critic2(action=actions,state=states), target_q_values.detach().unsqueeze(1)).mean()
        ...
        a = self.actor(states)
        actor_loss = -torch.mean(weights * self.critic1(action=a, state=states))  # Используем первый критик для обновления актера
